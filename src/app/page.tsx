...
export default async function HomePage() {
  ...
  return <AppFrame>
    ...
    <HomeMotion>
    <main>
      <section data-hero className="mx-auto max-w-6xl px-5 pb-12 pt-8 md:pb-16 md:pt-14">
        <div className="grid overflow-hidden rounded-[2rem] border bg-white lg:grid-cols-[1.05fr_.95fr]">
          <div className="flex flex-col justify-center p-7 md:p-12 lg:min-h-[590px] lg:p-14">
            <p data-hero-eyebrow className="text-xs font-medium uppercase tracking-[.24em] text-zinc-400">CoffeeBar · Anfu Road</p>
            <h1 className="display-title mt-6 text-6xl font-semibold md:text-8xl"><span className="block overflow-hidden"><span data-hero-line className="block">{headlineTop}</span></span><span className="block overflow-hidden"><span data-hero-line className="block">{headlineBottom}</span></span></h1>
            <p data-hero-copy className="mt-7 max-w-md text-sm leading-7 text-zinc-500 md:text-base">{t("认真做每一杯，也认真留出一点不着急的时间。线上点单，到店即取。")}</p>
          ...
          <div data-hero-visual className="relative min-h-[430px] overflow-hidden bg-black p-6 md:min-h-[520px] md:p-9 lg:min-h-[590px]">
            <div data-orbit="one" className="absolute -left-24 top-10 size-72 rounded-full bg-white/[.07] blur-3xl" />
            <div data-orbit="two" className="absolute -bottom-24 -right-16 size-80 rounded-full bg-amber-200/[.08] blur-3xl" />
            <div className="absolute inset-6 rounded-[1.6rem] border border-white/15 md:inset-9" />
            <div data-hero-visual-copy className="absolute left-10 top-10 z-10 md:left-14 md:top-14"><p className="text-xs font-medium uppercase tracking-[.22em] text-white/45">Today&apos;s coffee</p><p className="mt-2 text-xl font-medium text-white">{t("经典，从第一杯开始。")}</p></div>
      ...
      <section data-reveal className="mx-auto max-w-6xl px-5 pb-16">
        ...
        <div data-card-grid data-reveal-group className="relative grid grid-cols-2 gap-3 lg:grid-cols-4"><div data-card-glow className="pointer-events-none absolute left-0 top-0 z-0 rounded-[1.5rem] bg-black/10 opacity-0 blur-[1px]" />{coffeeShortcuts.map((item) => <TrackedLink data-card data-reveal-item href="/menu" eventName="home_cta_clicked" eventProperties={{ cta_name: "coffee_shortcut", shortcut_name: item.name, target_path: "/menu" }} key={item.name} className={cn("group relative z-10 overflow-hidden rounded-[1.5rem] border p-3 transition hover:-translate-y-1 hover:shadow-xl hover:shadow-black/5", item.tone)}>
      ...
      <section className="bg-black text-white">
        <div data-reveal-group className="mx-auto grid max-w-6xl gap-10 px-5 py-14 md:grid-cols-[1.2fr_1fr_1fr] md:py-20">
          <div data-reveal-item><p className="text-xs font-medium uppercase tracking-[.22em] text-white/40">Coffee, no rush</p><h2 className="mt-4 max-w-sm text-4xl font-semibold tracking-[-.05em]">{t("路过、停下，带走今天的一杯。")}</h2></div>
          <div data-reveal-item className="border-t border-white/15 pt-5 md:border-l md:border-t-0 md:pl-8 md:pt-0"><p className="font-mono text-3xl">15m</p><p className="mt-3 text-sm leading-6 text-white/55">{t("提前点单，预计十五分钟出杯，到店直接取走。")}</p></div>
          <div data-reveal-item className="border-t border-white/15 pt-5 md:border-l md:border-t-0 md:pl-8 md:pt-0"><p className="text-lg font-medium">10:00—21:30</p><p className="mt-3 text-sm leading-6 text-white/55">{t("每天营业，上海市安福路 108 号。")}</p></div>
        </div>
      </section>

      <section className="mx-auto max-w-6xl px-5 py-14 md:py-20">
        <div data-reveal className="flex flex-col gap-6 rounded-[2rem] border bg-white p-7 md:flex-row md:items-center md:justify-between md:p-12"><div><p className="text-xs font-medium uppercase tracking-[.22em] text-zinc-400">Objects for coffee</p><h2 className="mt-3 text-3xl font-semibold tracking-[-.045em]">{t("把咖啡日常带回家。")}</h2><p className="mt-3 max-w-xl text-sm leading-6 text-zinc-500">{t("杯子、滤杯和磨豆机，简洁耐用，支持门店自取。")}</p></div><TrackedLink data-magnetic href="/shop" eventName="home_cta_clicked" eventProperties={{ cta_name: "shop_section", target_path: "/shop" }} className="inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-full bg-black px-6 text-sm font-medium text-white">{t("进入商店")}<ArrowRight className="size-4" /></TrackedLink></div>
      </section>
    </main>
    </HomeMotion>
  </AppFrame>;
...
function CoffeeArtwork({ position, interactive = false }: { position: string; interactive?: boolean }) {
  return <div data-artwork={interactive ? "" : undefined} role="img" aria-label="手绘咖啡插画" className={cn("absolute inset-0 bg-no-repeat", interactive && "will-change-transform")} style={{ backgroundImage: "url('/illustrations/coffee-menu-handdrawn.png')", backgroundPosition: position, backgroundSize: "400% 300%" }} />;
}
